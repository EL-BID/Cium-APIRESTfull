...
* @created    2015-07-20
*
* Controlador `Plazo acción`: Manejo del catálogo del plazo para las acciones
*
*/
class PlazoAccionController extends Controller {
    ...
    {
        $this->middleware('permisos:GET.506C617A6F|POST.F6A716C605|PUT.F6A716C605|DELETE.F6A716C605');
    }
	/**
	...
	public function index()
	{
		...
		if(array_key_exists('pagina',$datos))
		{
			...
			if(array_key_exists('buscar',$datos))
			{
				...
				$search = trim($valor);
				$keyword = $search;
				$plazoAccion=$plazoAccion->whereNested(function($query) use ($keyword)
				{
			...
			{
				$plazoAccion = PlazoAccion::skip($pagina-1)->take($datos['limite'])->orderBy($order,$orden)->get();
